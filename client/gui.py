...
class GUI(threading.Thread):
    ...
    def run(self):
        ...
        self.C = Tkinter.Canvas(self.top, bg="#333333"  , height=height, width=width)


        # Create the grid
        for i in range(0, width, int(height/ND)):
            self.C.create_line([(i, 0), (i, height)], tag='grid_line', fill="#C8C8C8", )
            self.C.create_line([(0, i), (width, i)], tag='grid_line', fill="#C8C8C8")

        # Mic 0 Line
        micCoord0 =    baseWidth0, baseHeight0, baseWidth0 + radius * math.sin(math.radians(angle0)), baseHeight0 - radius * math.cos(math.radians(angle0))
        line0 = self.C.create_line(micCoord0, fill="#648FB1", width = 5, arrow = Tkinter.LAST, smooth = TRUE, tag='line')

        # Mic 1 Line
        micCoord1 =    baseWidth1, baseHeight1, baseWidth1 + radius * math.sin(math.radians(angle1)), baseHeight1 - radius * math.cos(math.radians(angle1))
        line1 = self.C.create_line(micCoord1, fill="#648FB1", width = 5, arrow = Tkinter.LAST, smooth = TRUE, tag='line')
        ...
        example = Tkinter.Label(self.top, text="Sonotrack Viz", font="Arial", width = 20, bg = "#D6B556", foreground = "#000000")
        ...
        DistanceText = Tkinter.Label(self.top, text="Array Distance", font="Arial", background = "#648FB1", fg = "white", height = 1)
        ...
        NumArrayText = Tkinter.Label(self.top, text="# Of Arrays", font="Arial", background = "#648FB1", fg = "white", height = 1)
        ...
        NumRunText = Tkinter.Label(self.top, text="# of Runs", font="Arial", background = "#648FB1", fg = "white", height = 1)
